  ...
  for (const sql of [
    ...
    try {
      ...
      if (sql.includes("available")) {
        db!.exec("UPDATE humans SET available = 1 WHERE available IS NULL");
      }
      if (sql.includes("show_email")) {
        db!.exec("UPDATE humans SET show_email = 0 WHERE show_email IS NULL");
      }
    } catch (error: any) {
...
export async function createHuman(params: { email: string; displayName?: string | null }): Promise<HumanRecord> {
  ...
  const info = db!.prepare(
    `INSERT INTO humans (email, display_name, headline, bio, city, state, country, skills, social_links, rate_per_hour, timezone, available, show_email, created_at) 
     VALUES (?, ?, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 1, 0, ?)`
...
export async function updateHuman(params: {
  id: number;
  ...
  try {
    db!.prepare(
      `UPDATE humans SET display_name = ?, headline = ?, bio = ?, city = ?, state = ?, country = ?, skills = ?, social_links = ?, rate_per_hour = ?, timezone = ?, available = ?, show_email = ? WHERE id = ?`
    ).run(displayName, headline, bio, city, state, country, skills, socialLinks, ratePerHour, timezone, available ? 1 : 0, showEmail ? 1 : 0, params.id);
  } catch (error: any) {
